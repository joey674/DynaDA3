...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        with torch.no_grad():
            with torch.autocast(device_type=image.device.type, dtype=autocast_dtype):
                logger.debug(f"images shape after forward in api.py: {image.shape}")
                return self.model(
    ...
    def inference(
        ...
        logger.debug(f"images shape after _preprocess_inputs in api.py 193: {imgs_cpu.shape}")

        # Prepare tensors for model
        imgs, ex_t, in_t = self._prepare_model_inputs(imgs_cpu, extrinsics, intrinsics)
        logger.debug(f"images shape after _prepare_model_inputs in api.py: {imgs.shape}")
